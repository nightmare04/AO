...
class PlaneCondition(QtWidgets.QWidget):
    def __init__(self, plane: PlaneM):
        ...
        self.removed_add_btn.clicked.connect(self.add_removed)
...
class AddDefect(QtWidgets.QWidget):
    def __init__(self, plane: PlaneM):
        ...
        self.subunit_label = QtWidgets.QLabel('Специальность:')
        self.subunit_name_edit = QtWidgets.QComboBox()
        subunits_list = list(SubunitM.select(SubunitM.name).where(SystemM.plane_type == self.plane.plane_type))
        subunits_name = map(lambda q: q.name, subunits_list)
        self.system_name_edit.addItems(subunits_name)
        self.main_layout.addWidget(self.system_name_label, 1, 0)
        self.main_layout.addWidget(self.system.name_edit, 1, 1)

        self.system_name_label = QtWidgets.QLabel('Система:')
        self.system_name_edit = QtWidgets.QComboBox()
        systems_list = list(SystemM.select(SystemM.name).where(SystemM.plane_type == self.plane.plane_type))
        systems_name = map(lambda q: q.name, systems_list)
        self.system_name_edit.addItems(systems_name)
        self.main_layout.addWidget(self.system_name_label, 1, 0)
        self.main_layout.addWidget(self.system.name_edit, 1, 1)

        self.type_label = QtWidgets.QLabel('Выберите блок / агрегат:')
        self.type_select = QtWidgets.QComboBox()
        agrs_list = list(AgregateM.select().where(
            AgregateM.id_system == AgregateM.get(AgregateM.name == self.system_name_edit.currentText())))
        agrs_name = map(lambda q: q.name, agrs_list)
        self.type_select.addItems(agrs_name)
        self.main_layout.addWidget(self.type_label, 2, 0)
        self.main_layout.addWidget(self.type_select, 2, 1)
